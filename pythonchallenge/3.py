...
# 1
# s[3] 与 s[max-3] 没有判断
# s[i-4] 和  s[i+4] 其实可以是非letter字符，但没有判断
def l(x):
    return x>='a' and x <= 'z'
def h(x):
    return x>='A' and x <= 'Z'
...
for i in range(4,len(s)-4):
    if h(s[i-3]) and h(s[i-2]) and h(s[i-1]) and h(s[i+1]) and h(s[i+2]) and h(s[i+3]) and l(s[i-4]) and l(s[i]) and l(s[i+4]):
        news+= s[i]
print news
